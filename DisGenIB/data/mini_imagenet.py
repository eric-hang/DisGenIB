...
_MINI_IMAGENET_DATASET_DIR = 'path to ImageNet'
...
class MiniImageNet_feature(data.Dataset):
    def __init__(self, phase='train', do_not_use_random_transf=False, use_base=True):
        ...
        if self.phase=='train':
            ...
            self.data = torch.load('path to train feature').cpu()
            self.labels = torch.load('path to train label').cpu().tolist()

            directory = 'path to category'
            classes = sorted(entry.name for entry in os.scandir(directory) if entry.is_dir())
            self.catname2label = {cls_name: i for i, cls_name in enumerate(classes)}
            ...
            data_base = torch.load('path to train feature').cpu()
            label_base = torch.load('path to train label').cpu()


            data_novel = torch.load('path to test feature').cpu()
            label_novel = torch.load('path to test label').cpu() + 80

                
            if use_base:
                self.data = torch.cat([data_base,data_novel],axis=0)
                self.labels = torch.cat([label_base,label_novel],axis=0).tolist()
            else:
                self.data = data_novel
                self.labels = label_novel.tolist()

            directory = 'path to category'
            classes = sorted(entry.name for entry in os.scandir(directory) if entry.is_dir())
            self.catname2label = {cls_name: i+80 for i, cls_name in enumerate(classes)}
            ...
            self.labelIds_base = buildLabelIndex(label_base.tolist()).keys()
            self.labelIds_novel = buildLabelIndex(label_novel.tolist()).keys()
            self.num_cats_base = len(self.labelIds_base)
...
class MiniImageNetPC_feature(data.Dataset):
    def __init__(self, phase='train', shot=1, do_not_use_random_transf=False, use_base=True):
        ...
        if self.phase == 'train':
            ...
            data_novel = torch.load('path to test or valid feature').cpu()
            label_novel = torch.load('path to test or valid label').cpu()+80
            ...
            if use_base:
                ...
                self.data = data_base
                self.labels = label_base.tolist()

            self.label2ind = buildLabelIndex(self.labels)
            self.labelIds = sorted(self.label2ind.keys())
            self.num_cats = len(self.labelIds)

            self.labelIds_base = buildLabelIndex(label_base).keys()
            self.labelIds_novel = buildLabelIndex(label_novel).keys()
            self.num_cats_base = len(self.labelIds_base)
            self.num_cats_novel = len(self.labelIds_novel)
            intersection = set(self.labelIds_base) & set(self.labelIds_novel)
            assert (len(intersection) == 0)
